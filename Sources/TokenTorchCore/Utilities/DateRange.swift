...
public enum DateRange {
        ...
        let (startStr, autoEnd): (String, String?) = try {
            guard let input = startInput else {
                ...
                guard let year = comps.year, let month = comps.month else {
                    throw TokenTorchError.message("Failed to create start of current month")
                }
            ...
            switch parts.count {
                case 1:
                    guard let year = Int(parts[0]) else { throw TokenTorchError.message("Invalid year format") }
                    let start = try firstDay(year: year, month: 1)
                    var endComps = DateComponents()
                    endComps.year = year
                    endComps.month = 12
                    endComps.day = 31
                    guard let end = calendar.date(from: endComps) else { throw TokenTorchError.message("Invalid year") }
                    return (formatter.string(from: start), formatter.string(from: end))
                case 2:
                    guard let year = Int(parts[0]), let month = Int(parts[1]) else {
                        throw TokenTorchError.message("Invalid year-month format")
                    }
                    let start = try firstDay(year: year, month: month)
                    let end = try lastDayOfMonth(year: year, month: month)
                    return (formatter.string(from: start), formatter.string(from: end))
                case 3:
                    guard let year = Int(parts[0]), let month = Int(parts[1]), let day = Int(parts[2]) else {
                        throw TokenTorchError.message("Invalid date format")
                    }
                    var comps = DateComponents()
                    comps.year = year
                    comps.month = month
                    comps.day = day
                    guard let start = calendar.date(from: comps) else { throw TokenTorchError.message("Invalid date") }
                    return (formatter.string(from: start), nil)
                default:
                    throw TokenTorchError.message("Invalid date format. Use YYYY, YYYY-MM, or YYYY-MM-DD")
            }
        }()
    ...
    public static func lastDayOfMonth(year: Int, month: Int) throws -> Date {
        ...
        guard let nextMonth = calendar.date(from: comps),
            ...
            throw TokenTorchError.message("Failed to get last day of month")
        }
        return last
    }
    ...
    public static func exclusiveDayAfter(end: String) throws -> Date {
        ...
        guard let date = formatter.date(from: end),
            ...
            throw TokenTorchError.message("Invalid end date: \(end)")
        }
        return next
    }
    ...
    public static func startToRFC3339(start: String) throws -> String {
        ...
        guard formatter.date(from: start) != nil else {
            throw TokenTorchError.message("Invalid start date: \(start)")
        }
    ...
    private static func dateToUnix(_ dateStr: String) throws -> Int64 {
        ...
        guard let date = formatter.date(from: dateStr) else {
            throw TokenTorchError.message("Invalid date: \(dateStr)")
        }
