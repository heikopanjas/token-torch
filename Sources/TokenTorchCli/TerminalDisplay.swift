...
enum TerminalDisplay {
    ...
    static func displaySubscriptionQuota(_ report: SubscriptionQuotaReport) {
        ...
        if isCursor {
            ...
            if let message = report.rawMessage {
                ...
                print(
                    ANSIColor.dimmed("This is includedSpend ÷ API allowance at provider rates. It is separate from the percentage meters above."))
            }
        }
        else {
    ...
    static func displayCursorOrgUnavailable() {
        print()
        print(ANSIColor.yellow("Cursor organization billing is not available in \(AppBrand.displayName)."))
        print(ANSIColor.dimmed("Use `token-torch-cli cursor --quota` for personal subscription plan usage (macOS local OAuth)."))
        print()
    }
    ...
    static func displayCopilotOrgUnavailable() {
        ...
        print(
            ANSIColor.dimmed(
                "Use `token-torch-cli copilot --quota` for personal subscription quota (fine-grained GitHub PAT with Copilot requests permission)."
            ))
        print()
    }
