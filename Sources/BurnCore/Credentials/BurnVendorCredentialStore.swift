...
/// Burn-owned Keychain storage for imported vendor OAuth sessions (`com.burn.vendor.*`).
public enum BurnVendorCredentialStore {
    private static let account = "burn"

    public static func service(for provider: ProviderID) -> String {
        "com.burn.vendor.\(provider.rawValue).oauth"
    }
    ...
    private static func encodeSession(_ session: OAuthSession) throws -> String {
        ...
        guard let json = String(data: data, encoding: .utf8) else {
            throw BurnError.message("Failed to encode vendor credential copy")
        }
        return json
    }
