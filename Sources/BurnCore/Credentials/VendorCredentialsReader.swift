...
public enum VendorCredentialsReader {
    ...
    public static func quotaSessionExpired(provider: String, session: OAuthSession, vendorAction: String) -> BurnError {
        .message(
            "\(provider) access token expired or invalid (read from \(session.sourceLabel())). \(vendorAction) burn never writes to vendor credential stores."
        )
    }
    ...
    public static func loadClaudeSession(strategy: VendorCredentialStrategy = .directVendorRead) throws -> OAuthSession {
        switch strategy {
            ...
            case .burnOwnedCopy:
                return try loadBurnCopySession(provider: .claude, missingMessage: claudeMissingMessage)
        }
    }
    ...
    public static func loadCodexSession(strategy: VendorCredentialStrategy = .directVendorRead) throws -> OAuthSession {
        switch strategy {
            ...
            case .burnOwnedCopy:
                return try loadBurnCopySession(provider: .codex, missingMessage: codexMissingMessage)
        }
    }
    ...
    public static func loadCursorSession(strategy: VendorCredentialStrategy = .directVendorRead) throws -> OAuthSession {
        switch strategy {
            ...
            case .burnOwnedCopy:
                return try loadBurnCopySession(provider: .cursor, missingMessage: cursorMissingMessage)
        }
    }

    // MARK: - Burn copy load path

    private static func loadBurnCopySession(provider: ProviderID, missingMessage: String) throws -> OAuthSession {
        if let cached = VendorCredentialCache.session(for: provider) {
            return cached
        }
        guard let session = try BurnVendorCredentialStore.load(provider: provider) else {
            throw BurnError.missingCredentials(missingMessage)
        }
    ...
    private static func loadClaudeSessionDirect() throws -> OAuthSession {
        ...
        guard let best = candidates.max(by: { freshness($0) < freshness($1) }) else {
            throw BurnError.missingCredentials(claudeMissingMessage)
        }
        return best
    }
    ...
    private static func loadCodexSessionDirect() throws -> OAuthSession {
        ...
        throw BurnError.missingCredentials(codexMissingMessage)
    }
    ...
    private static func loadCursorSessionDirect() throws -> OAuthSession {
        ...
        throw BurnError.missingCredentials(cursorMissingMessage)
    }
    ...
    private static func readSQLiteValue(dbPath: URL, key: String) throws -> String? {
        ...
        guard sqlite3_open_v2(dbPath.path, &db, SQLITE_OPEN_READONLY, nil) == SQLITE_OK, let db else {
            throw BurnError.message("Failed to open Cursor state database at \(dbPath.path). Close Cursor and retry.")
        }
