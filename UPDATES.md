...
- migrated entries keep the older `### date: Title` heading with **What** / **Why** / **How** paragraphs; entries written from here on use the `recent-updates` format
- AGENTS.md now ends after "Semantic Versioning" and carries instructions only, dropping it from about 111 KB to about 25 KB
- rationale: every session loaded roughly 930 lines of changelog as part of the primary instruction file, and the log already lived here
- no version bump: documentation only, no behavior change

### 2026-09-01 18:11 (v5.9.0, point agents.md at updates.md)

- the update protocol and documentation sections of AGENTS.md now send new log entries to UPDATES.md and reference the `recent-updates` skill instead of a log section inside AGENTS.md
- the swift coding conventions pointer moves from `.agents/skills/` to `.claude/skills/`, matching the agent tooling migration in 7c40e93
- rationale: AGENTS.md still described itself as the home of the changelog after the log moved to UPDATES.md, so the two files disagreed about where updates belong
- no version bump: documentation only, no behavior change
...
### 2026-08-05: Claude repair targets imported config + controlling terminal

**What**: Claude credential repair now sets `CLAUDE_CONFIG_DIR` from the imported credential source (hashed Keychain service / credentials file) and launches the repair shell through a self-exec PTY bootstrap that creates a real controlling terminal session. Failure diagnostics include secret-safe auth-selector states and the selected config directory.

**Why**: GUI LaunchServices environments often lack the user's shell `CLAUDE_CONFIG_DIR`, so repair was refreshing the default `~/.claude` profile while Token Torch polled a custom `~/.config/claude` Keychain namespace. `isatty` alone was also insufficient versus a session leader with `/dev/tty`.

**How**: `ClaudeCredentialPaths`, `ClaudeCredentialRepair`, `PseudoTerminalChildBootstrap`, `ProcessRunner` (`POSIX_SPAWN_SETSID`), tests, README, AGENTS. Version `5.8.6`.

### 2026-08-05: Claude repair via pseudo-terminal

**What**: Claude credential repair now launches `ANTHROPIC_API_KEY="" exec claude -p "/usage"` inside a pseudo-terminal so Claude Code sees a TTY on stdin/stdout/stderr. Also fixed the automatic-repair importer fall-through so opted-in background refreshes continue into repair instead of returning a needs-authorization notice.

**Why**: Pipe-backed subprocesses do not satisfy Claude Code's terminal requirement for subscription credential refresh; the verified scratch prototype refreshes only when run under `openpty`. The orchestrator branch previously inverted the automatic-repair continue helper.

**How**: `ProcessRunner.runInPseudoTerminal`, `ClaudeCredentialRepair`, `UsageOrchestrator.shouldReturnNeedsAuthorizationAfterImporterFailure`, tests, README, AGENTS. Version `5.8.5`.
