...
Last updated: 2026-09-03 (desktop notification when a capped row's usage enters the orange/red bands, configurable in a new Notifications settings tab)
...
- `AppNotification` / `NotificationService` — general desktop notification content model (`AppNotification` factories per use case) and app-layer poster (`NotificationService.bootstrap()` requests authorization on first launch and posts a welcome notification on grant; `post(_:)` delivers any notification). Consumers: Claude repair failure on automatic refreshes, gated by `ProviderPreferences.notifyOnRepairFailure` (default on); and usage-threshold alerts (see "Usage-threshold notifications" above), gated by `ProviderPreferences.notifyOnUsageThreshold` (default on, Settings → Notifications tab). Extend by adding new `AppNotification` factories only
...
- `UsageLevel.level(forPercent:)`, `UsageBarMetrics.showsBar(forPercent:)` and `UsageBarMetrics.fillFraction(forPercent:)` in `token-torch/Core/Models/QuotaModels.swift` hold the pure logic (AppKit-free, unit-tested). Bands use exclusive upper bounds derived from `UsageLevel.thresholdPercent` (0/50/75/87/95): `<50` light green, `<75` dark green, `<87` orange, `<95` light red, else dark red. The fill fraction is clamped because Copilot counts overage past 100%. `UsageLevel` is `String`-backed and `Comparable` so it can be persisted and ranked (see "Usage-threshold notifications" below) without a stored value silently reinterpreting after a future band is added.
...
- `CappedUsageRows` (`token-torch/Core/Models/CappedUsageRows.swift`) enumerates every row of a fetch result that states a share of a cap — exactly the rows the bar above attaches to, via `ProviderPreferences.visibleWindows` and the `cappedUsedPercent`/`creditsRowPercent`/`cursorCreditsPercent` properties. Each row's `UsageAlertRowKey` deliberately carries no percentage or `resetsAt` (both rotate every window) and disambiguates same-label duplicates (e.g. two Codex additional limits with no `limit_name`) with an `occurrence` counter.
- `UsageAlertEvaluator` (`token-torch/Core/Services/UsageAlertEvaluator.swift`) is a pure state machine: given the current rows and the last-persisted `UsageAlertState`, it alerts a row the first time it reaches `ProviderPreferences.usageAlertStartLevel` (Settings → Notifications tab; Orange `.high` or Red `.severe`) and again on each higher band, never twice in the same band. Falling back below the start band clears that row's stored band, so climbing back up alerts again. A row's stored band is pruned only when its section reported a usable `.subscription` result this refresh and the row is genuinely gone (a display gate turned off) — a transient `.error`/`.needsAuthorization` leaves it untouched so it can't re-arm on recovery. `UsageAlertStateStore` persists it under `AppBrand.usageAlertStateKey`, decoding leniently ([String: String] → `compactMapValues(UsageLevel.init(rawValue:))`) so an unrecognized band from a future build drops one entry instead of wiping the whole dictionary.
- `MenuBarViewModel.evaluateUsageAlerts` runs this on **every** refresh, interactive or not, and always saves the resulting state — even when `ProviderPreferences.notifyOnUsageThreshold` is off — so enabling the toggle mid-band doesn't immediately burst a backlog of alerts, and an interactive refresh that first observes a new band doesn't get re-announced by the next timer tick. Posting is the only part gated by the toggle. `AppNotification.usageThresholdReached(section:rows:)` posts at most one banner per provider section per refresh, with a per-section identifier so a later escalation replaces the earlier banner instead of stacking.
...
**The General tab is the one exception that cannot just "make it scroll."** `SettingsWindowController.init` sets `generalController` as the window's initial `contentViewController` before any `fitWindow` call, so `SettingsStyle.generalPaneHeight` **is** the window's content height at launch — raising it to fit new content grows the window every launch, not just that tab. A feature that needs new General-tab controls belongs on its own toolbar tab instead (see the Notifications tab, added for usage-threshold alerts) rather than extending `generalPaneHeight`.
